...
export function libraryRequest(params: URLSearchParams) {
  ...
  filters.delete('problem_id');
  filters.set('limit', '250');
  return { path: `/api/problems?${filters}`, single: false };
}

export async function fetchLibrarySelection(
  params: URLSearchParams,
  request: <T>(path: string) => Promise<T>,
) {
  const { path, single } = libraryRequest(params);
  if (single) {
    const problem = await request<Problem>(path);
    return { items: [problem], total: 1 };
  }
...
export function updateLibraryFilter(params: URLSearchParams, key: string, value: string) {
  ...
  next.delete('problem_id');
  if (key === 'main_id') {
    next.delete('subtag_id');
    next.delete('taxonomy_id');
  }
