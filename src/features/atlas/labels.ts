...
export function placeSpatialLabels(candidates: LabelCandidate[], width: number, height: number) {
  ...
  for (const candidate of candidates) {
    for (const [dx, dy] of [
      [0, -38],
      [0, 20],
      [candidate.width / 2 + 20, -10],
      [-candidate.width / 2 - 20, -10],
    ]) {
      const x = candidate.x + dx - candidate.width / 2,
        y = candidate.y + dy;
      ...
      if (
        occupied.some(
          (box) =>
            ...
            x + candidate.width + 10 > box.x &&
            y < box.y + 40 &&
            y + 40 > box.y,
        )
      )
        continue;
      occupied.push({ x, y, width: candidate.width });
      result.push({ id: candidate.id, x: x + candidate.width / 2, y });
      break;
    }
  }
  return result;
