...
export function requestSpatialLayout(graph: LayoutInput): Promise<LayoutResult> {
  ...
  const promise = new Promise<LayoutResult>((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      worker.terminate();
      reject(new Error('The spatial layout took too long. Use 2D or try again.'));
    }, 15000);
    const finish = () => {
    ...
    worker.onerror = () => {
      finish();
      reject(new Error('The 3D layout is unavailable. You can still use 2D or List.'));
    };
    try {
...
export function useSpatialLayout(graph: LayoutInput, enabled: boolean) {
  ...
  const [state, setState] = useState<{ key: string; result?: LayoutResult; error?: string }>({
    key: '',
  });
  useEffect(() => {
    if (!enabled) return;
    let current = true;
    requestSpatialLayout(graph).then(
      (result) => {
        if (current) setState({ key, result });
      },
      (error) => {
        if (current) setState({ key, error: String(error.message) });
      },
    );
    return () => {
      current = false;
