...
export type StudyNode = AtlasNode & {
  x: number;
  y: number;
  width: number;
  height: number;
...
export function wrapLabel(text: string, max = 26) {
  ...
  for (const token of text.split(/\s+/))
    for (let start = 0; start < token.length; start += max) {
      const word = token.slice(start, start + max);
...
export function layout2D(graph: LayoutInput, wide: boolean, compact = false) {
  ...
  const columns = wide ? 2 : 1,
    groupWidth = compact ? 340 : 568,
    gap = 64,
    heights = Array(columns).fill(0) as number[];
  const place = (node: AtlasNode, x: number, y: number, width: number, max: number) => {
    const lines = wrapLabel(node.name, max),
      height = Math.max(44, lines.length * 17 + 18);
    const result = { ...node, x, y, width, height, lines };
    nodes.push(result);
    placed.add(node.id);
    return result;
  };
  model.domains.forEach((domain, index) => {
    const col = index % columns,
      x = col * (groupWidth + gap),
      y = heights[col];
    const heading = place(domain, x, y, groupWidth, compact ? 27 : 52);
    let row = y + heading.height + 52;
    const techniques = (model.children.get(domain.id) ?? [])
      .map((id) => model.byId.get(id)!)
      .filter(Boolean)
      .sort((a, b) => a.name.localeCompare(b.name));
    for (const technique of techniques) {
      const problems = (model.children.get(technique.id) ?? [])
        .map((id) => model.byId.get(id)!)
        .filter(Boolean)
        .sort((a, b) => a.name.localeCompare(b.name));
      const start = row;
    ...
    const height = Math.max(130, row - y);
    groups.push({
      id: domain.id,
      x,
      y,
      ...
      count: model.problemIds(domain.id).length,
    });
    heights[col] += height + 56;
  });
  let footer = Math.max(0, ...heights) + 16;
  for (const node of graph.nodes
    .filter((node) => !placed.has(node.id))
    .sort((a, b) => a.name.localeCompare(b.name))) {
    const item = place(node, compact ? 8 : 32, footer, compact ? 326 : 500, compact ? 32 : 52);
    footer += item.height + 20;
  }
  return {
    nodes,
    groups,
    width: columns * (groupWidth + gap) - gap,
    height: Math.max(160, footer),
    links: graph.links,
  };
}
export function studyEdge(source: StudyNode, target: StudyNode, kind: string) {
  ...
  const x1 = source.x + source.width + 4,
...
export function fitStudyCamera(nodes: StudyNode[], width: number, height: number) {
  ...
  const zoom = Math.min(
    1,
    Math.max(1, width - 48) / Math.max(1, maxX - minX),
    Math.max(1, height - 300) / Math.max(1, maxY - minY),
  );
  return { zoom: Math.max(0.0001, zoom), x: 24 - minX * zoom, y: 168 - minY * zoom };
}
