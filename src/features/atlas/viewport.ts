...
export function zoomAtlasAt(
  previous: { zoom: number; x: number; y: number },
  factor: number,
  x: number,
  y: number,
  centerX: number,
  ...
  const zoom = clampAtlasZoom(previous.zoom * factor),
    ratio = zoom / previous.zoom;
  return {
    zoom,
    x: x - centerX - (x - centerX - previous.x) * ratio,
    y: y - centerY - (y - centerY - previous.y) * ratio,
  };
}
// Preserve the point under the gesture midpoint as the fingers zoom and translate.
export function pinchAtlasAt(
  ...
  return {
    ...result,
    x: result.x + (next.x - previous.x) / 2,
    y: result.y + (next.y - previous.y) / 2,
  };
}
...
export function spatialCamera(nodes: PositionedNode[], width: number, height: number) {
  ...
  const target = { ...bounds.center, y: bounds.center.y + bounds.radius * 0.08 };
  const direction = { x: 0.35, y: 0.2, z: 1 },
