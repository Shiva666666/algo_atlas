...
export function createAtlasModel(atlas: AtlasGraphData) {
  ...
  for (const link of links) {
    ...
    if (link.kind === 'hierarchy' || link.kind === 'primary') {
      ...
      parent.set(link.target, link.source);
    }
  }
  function problemIds(nodeId: string): string[] {
    ...
    visit(nodeId);
    if (['pattern', 'custom'].includes(byId.get(nodeId)?.kind ?? ''))
      for (const id of neighbors.get(nodeId) ?? [])
        if (byId.get(id)?.kind === 'problem') found.add(id);
    return [...found];
  }
  const domains = atlas.nodes
    .filter((node) => node.kind === 'main')
...
export function selectAtlasGraph(
  model: AtlasModel,
  mode: 'study' | 'all',
  query = '',
  status = 'all',
) {
  const eligible = model.nodes.filter(
    (node) => mode === 'all' || model.problemIds(node.id).length > 0,
  );
  const matched = eligible.filter((node) =>
    atlasNodeMatches(node, query, status, model.problemIds, model.byId),
  );
  const ids = new Set(matched.map((node) => node.id));
  for (const node of matched)
    if (query.trim())
      for (const id of model.problemIds(node.id))
        if (status === 'all' || model.byId.get(id)?.status === status) ids.add(id);
  for (const id of [...ids]) {
    let current = id;
    const seen = new Set<string>();
    ...
    if (model.byId.get(id)?.kind === 'problem')
      for (const neighbor of model.neighbors.get(id) ?? [])
        if (['pattern', 'custom'].includes(model.byId.get(neighbor)?.kind ?? '')) ids.add(neighbor);
  }
  return {
...
export function atlasNodeMatches(
  node: AtlasNode,
  query: string,
  scope: string,
  problemIds: (id: string) => string[],
  ...
  const hasStatus =
    ...
    (node.kind === 'problem'
      ? node.status === scope
      : problemIds(node.id).some((id) => byId.get(id)?.status === scope));
