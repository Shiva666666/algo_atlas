...
export function graphIdentity(graph: LayoutInput) {
  return JSON.stringify([
    'spatial-v1',
    graph.nodes.map((n) => [n.id, n.kind]).sort(),
    graph.links.map((l) => [l.source, l.target, l.kind]).sort(),
  ]);
}
export const isCurrentResult = (
  message: LayoutResponse,
  request: Pick<LayoutRequest, 'requestId' | 'key'>,
) => message.requestId === request.requestId && message.key === request.key;
// Pick only serializable graph data; the relationship model also contains functions.
export function createLayoutRequest(graph: LayoutInput, requestId: number): LayoutRequest {
  return {
    requestId,
    key: graphIdentity(graph),
...
export function positionedNodes(nodes: AtlasNode[], layout: LayoutResult): PositionedNode[] {
  return nodes.flatMap((node) => {
    const p = layout.positions[node.id];
    return p ? [{ ...node, ...p, fx: p.x, fy: p.y, fz: p.z }] : [];
  });
}
