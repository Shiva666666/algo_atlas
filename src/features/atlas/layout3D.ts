...
export const nodeRadius = (kind: string) =>
...
export function layout3D(graph: LayoutInput): LayoutResult {
  ...
  [...model.domains]
    ...
    .forEach((domain, index) => {
      const vector = direction(index, model.domains.length),
        radius = spread * (index % 3 === 0 ? 0.62 : 1);
      anchors.set(domain.id, { x: vector.x * radius, y: vector.y * radius, z: vector.z * radius });
    });
  const rootFor = (id: string) => {
    let current = id;
    const seen = new Set<string>();
    while (model.parent.has(current) && !seen.has(current)) {
      seen.add(current);
  ...
  const nodes = sorted.nodes.map((node, index) => {
    const random = seed(node.id),
      vector = direction(index, sorted.nodes.length);
    const root = anchors.get(rootFor(node.id));
    const anchor =
      anchors.get(node.id) ??
      (root
        ? {
            x: root.x + (random() - 0.5) * 135,
            y: root.y + (random() - 0.5) * 135,
            z: root.z + (random() - 0.5) * 135,
          }
        : { x: vector.x * spread * 0.5, y: vector.y * spread * 0.5, z: vector.z * spread * 0.5 });
    anchors.set(node.id, anchor);
    return { ...node, ...anchor };
  });
  for (const node of nodes)
    if (['pattern', 'custom'].includes(node.kind)) {
      const adjacent = [...(model.neighbors.get(node.id) ?? [])]
        .map((id) => anchors.get(id))
        .filter((p): p is Point3 => !!p);
      if (adjacent.length) {
        const average = { x: 0, y: 0, z: 0 };
        for (const p of adjacent)
          for (const axis of ['x', 'y', 'z'] as const) average[axis] += p[axis] / adjacent.length;
        anchors.set(node.id, average);
        Object.assign(node, average);
      }
    }
  const simulation = forceSimulation(nodes, 3)
    .stop()
    .randomSource(seed('atlas-layout-v1'))
    .force(
      'links',
      forceLink(model.links.map((link) => ({ ...link })))
        .id((n: { id: string }) => n.id)
        .distance((l: { kind: string }) =>
          l.kind === 'pattern' ? 145 : l.kind === 'hierarchy' ? 95 : 65,
        )
        .strength((l: { kind: string }) => (l.kind === 'pattern' ? 0.04 : 0.3)),
    )
    .force('charge', forceManyBody().strength(-105))
    .force('collide', forceCollide((n: { kind: string }) => nodeRadius(n.kind) + 15).iterations(2))
    .force('x', forceX((n: { id: string }) => anchors.get(n.id)!.x).strength(0.075))
    .force('y', forceY((n: { id: string }) => anchors.get(n.id)!.y).strength(0.075))
    .force('z', forceZ((n: { id: string }) => anchors.get(n.id)!.z).strength(0.075))
    .alphaDecay(0.025);
  simulation.tick(280);
  simulation.stop();
  const raw = layoutBounds(nodes);
  const spans = ['x', 'y', 'z'].map(
    (axis) => raw.max[axis as keyof Point3] - raw.min[axis as keyof Point3],
  );
  ...
  for (const node of nodes) {
    ...
    for (const [i, axis] of (['x', 'y', 'z'] as const).entries())
      p[axis] =
        (node[axis] - raw.center[axis]) *
        (nodes.length >= 6 ? Math.min(1.8, target / Math.max(spans[i], 1)) : 1);
    positions[node.id] = p;
  }
