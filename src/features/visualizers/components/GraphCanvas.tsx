import type { GraphEdgeView, GraphNodeView } from '../core/types';
...
export function GraphCanvas({ nodes, edges, label }: GraphCanvasProps) {
  const points = new Map(
    nodes.map((node, index) => [node.id, position(node, index, nodes.length)]),
  );
  return (
    <svg className="learning-graph" viewBox="0 0 640 340" role="img" aria-label={label}>
      <title>{label}</title>
      ...
      <g className="learning-graph-edges">
        {edges.map((edge, index) => {
          ...
          return (
            <g className={`edge-${edge.state}`} key={`${edge.from}-${edge.to}-${index}`}>
        ...
        {nodes.map((node, index) => {
          ...
          return (
            ...
            >
              ...
              {node.detail && (
                <text className="node-detail" y={37}>
                  {node.detail}
                </text>
              )}
