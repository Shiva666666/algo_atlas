...
export function InsightRail({ insights }: { insights: InsightModel }) {
  const items = [
    ...
    ['BASE', insights.base],
    ['CHOICE', insights.choice],
    ['INVARIANT', insights.invariant],
  ];
  return (
...
export function RuleStrip({ rules }: { rules: RuleFocus[] }) {
  return (
    <div className="rule-strip" aria-label="Algorithm tokens aligned to the current visual">
      {rules.map((rule, index) => (
        <div className={rule.active ? 'active' : ''} key={`${rule.token}-${index}`}>
