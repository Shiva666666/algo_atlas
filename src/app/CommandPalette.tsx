...
import { Activity, Atom, BookOpen, Hexagon, Plus, Search, Settings2, X } from 'lucide-react';
...
export function CommandPalette({
  ...
  const { data, isFetching } = useQuery({
    queryKey: queryKeys.commandSearch(query),
    queryFn: () =>
      api<{ items: Problem[] }>(`/api/problems?q=${encodeURIComponent(query)}&limit=8`),
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="command-dialog">
          ...
          <Dialog.Description className="sr-only">
            Search saved problems or choose a destination. Results update as you type.
          </Dialog.Description>
          ...
          <div className="command-results" aria-live="polite">
            ...
            {data?.items.map((problem) => (
              <button key={problem.id} onClick={() => go(`/problems/${problem.id}`)}>
                <i
            ...
            {query.length > 1 && !isFetching && data?.items.length === 0 && (
              <div className="command-empty">No signal found. Log it as a new mistake.</div>
            )}
          </div>
        </Dialog.Content>
      </Dialog.Portal>
