...
export interface TaxonomyResponse {
  ...
  main: TaxonomyNode[];
  sub: TaxonomyNode[];
  patterns: TaxonomyNode[];
  failure_reasons: TaxonomyNode[];
}
export interface MistakeEvent {
  id: string;
  occurred_at: string;
  observation: string;
  reasons: TaxonomyNode[];
}
export interface Problem {
  ...
  primary_subtag: TaxonomyNode;
  primary_main: TaxonomyNode;
  ...
  mistake_events?: MistakeEvent[];
}
export interface ProblemPayload {
  record_initial_mistake?: boolean;
  title: string;
  url?: string | null;
  source?: string;
  source_key?: string;
...
export interface Analytics {
  ...
  domains: Array<TaxonomyNode & { count: number; open: number; repeat_mistakes: number }>;
...
export interface GitState {
  ...
  warnings: string[];
  changes?: Array<{ code: string; path: string; kind: string }>;
  additions?: number;
  updates?: number;
  deletions?: number;
...
export interface SyncRecordSummary {
  id: string;
  title: string;
  updated_at: string;
}
export interface SyncFieldComparison {
  field: string;
  label: string;
  local: string;
  incoming: string;
