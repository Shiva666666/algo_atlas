import { useReducedMotionPreference as useLessonReducedMotion } from '../hooks/useReducedMotionPreference';
export { useReducedMotionPreference as useLessonReducedMotion } from '../hooks/useReducedMotionPreference';
...
export function StateLegend({
  ...
  return (
    <ul className="state-legend" aria-label="Visual key" onMouseLeave={() => setHoveredIndex(null)}>
      {items.map((item, index) => (
        <li
          key={item.label}
          onMouseEnter={() => setHoveredIndex(index)}
          style={{ opacity: hoveredIndex === null || hoveredIndex === index ? 1 : 0.65 }}
        >
          <span className="state-legend-marker" style={{ color: item.color }} aria-hidden="true">
            {item.symbol}
          </span>
          {item.label}
        </li>
...
export function SmoothTabs({
  ...
  return (
    <div className="smooth-tabs" ref={container} role="tablist" aria-label={label}>
      <motion.div
