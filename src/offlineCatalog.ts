...
export const offlineProblems:Problem[]=[
  ...
  problem('leetcode','maximal-square','Maximal Square','https://leetcode.com/problems/maximal-square/','Medium','Resolved',subtags.gridDp,['tabulation'],intuition('dp[i][j] is the largest all-1 square side ending at cell (i,j).','Initialize the first row and column because the interior loop skips them.','For each interior 1, use 1 + min(diagonal, top, left).','Every referenced neighbor has already been solved.',['Single cell','All zeros','Only a boundary 1']),'O(m × n)','O(m × n)'),
  problem('leetcode','is-graph-bipartite','Is Graph Bipartite?','https://leetcode.com/problems/is-graph-bipartite/','Medium','Resolved',subtags.graph,['bfs','dfs'],intuition('Each vertex is uncolored, side A, or side B.','Seed every disconnected component with either side.','Color an unseen neighbor opposite; reject a same-color edge.','Every processed edge joins opposite colors.',['Odd cycle','Disconnected graph','Isolated node']),'O(V + E)','O(V)'),
  problem('leetcode','subsets-ii','Subsets II','https://leetcode.com/problems/subsets-ii/','Medium','Resolved',subtags.subsets,['sorting'],intuition('Skip equal sibling branches after sorting; allow equal values deeper.','Every current path is already a valid subset.','Choose, recurse with the next index, then remove.','One recursion level never starts two branches with the same value.',['All duplicates','Empty input']),'O(n log n + n × unique subsets)','O(n) auxiliary + output'),
  problem('geeksforgeeks','generate-all-possible-parentheses','Generate Parentheses','https://www.geeksforgeeks.org/problems/generate-all-possible-parentheses/1','Medium','Resolved',subtags.subsets,[],intuition('Build a valid prefix and stop any branch as soon as closed exceeds opened.','Start with one opening parenthesis.','Append either parenthesis; record only length-n strings with equal counts.','Every explored prefix has opened ≥ closed and length ≤ n.',['Smallest n=2','Only even lengths finish balanced']),'O(C(n/2) × n)','O(n) auxiliary + output'),
