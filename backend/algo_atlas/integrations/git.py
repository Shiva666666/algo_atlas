...
def configure_git(
    remote_url: str | None, branch: str, user_name: str | None, user_email: str | None
) -> dict:
    if remote_url:
...
def git_state(fetch: bool = False) -> dict:
    ...
    branch = (
        _value("config", "--get", "algoatlas.branch")
        or _value("branch", "--show-current")
        or "main"
    )
    ...
    if has_head and has_remote_branch:
        ...
        if behind:
            warnings.append(
                "The remote branch has unseen commits. Pull and resolve them before publishing."
            )
    return {
        "remote": remote,
        "branch": branch,
    ...
    state.update(
        {
            ...
            "ready": not state["warnings"]
            and (
                bool(changes)
                or state["ahead"] > 0
                or (state["has_head"] and not state["remote_branch_exists"])
            ),
        }
    )
    return state
...
def publish_git() -> dict:
    ...
    if pushed.returncode != 0:
        raise RuntimeError(
            f"The local commit was preserved, but GitHub rejected the push: {pushed.stderr.strip()}"
        )
