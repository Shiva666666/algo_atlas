...
def record_directory(record: dict) -> Path:
...
def validate_export_catalog() -> dict:
    ...
    for index, record in enumerate(records):
        ...
        for filename, hash_key in (
            ("README.md", "markdown_sha256"),
            ("solution.py", "solution_sha256"),
        ):
        ...
        metadata, _notes = parse_markdown(directory / "README.md")
...
def _markdown(problem: dict) -> str:
    yaml_text = yaml.safe_dump(
        _frontmatter(problem), sort_keys=False, allow_unicode=True, default_flow_style=False
    ).strip()
...
def export_catalog(session: Session) -> dict:
    ...
    try:
        ...
        for problem_id in problem_ids:
            ...
            relative = (
                Path(item["primary_main"]["slug"]) / item["primary_subtag"]["slug"] / item["slug"]
            )
            ...
            solution = (
                item["python_code"].replace("\r\n", "\n").rstrip() + "\n"
                if item["python_code"].strip()
                else "# Add your Python solution here.\n"
            )
            ...
            records.append(
                {
                    ...
                    "markdown_sha256": _sha(markdown_path),
                    "solution_sha256": _sha(solution_path),
                }
            )
        taxonomy = [
            taxonomy_to_dict(node)
            for node in session.scalars(
                select(TaxonomyNode).order_by(
                    TaxonomyNode.kind, TaxonomyNode.sort_order, TaxonomyNode.name
                )
            ).all()
        ]
        catalog = {
...
def parse_markdown(path: Path) -> tuple[dict, dict[str, list[str]]]:
