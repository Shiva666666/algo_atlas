...
LEARNING_PACK: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "status": "Resolved",
        "primary_subtag": "subsets",
        "taxonomy": ["constraint-search"],
        "failure_reasons": [],
        "time_complexity": "O(C(n/2) × n)",
        "space_complexity": "O(n) auxiliary + output",
        "observation": "Solved successfully on the first submission: 22/22 test cases passed.",
        "notes": {
            "why_missed": ["No recorded miss; the submission passed all test cases on the first attempt."],
            "recognition_signals": ["Build a constrained sequence one character at a time and reject a prefix as soon as it cannot become valid."],
            "core_insight": ["A valid prefix never has more closing than opening parentheses, and a completed answer has equal counts."],
            "approach": ["Start with one opening parenthesis.", "Prune when closed exceeds opened or the string exceeds n characters.", "Record a string at length n only when both counts match.", "Otherwise try appending an opening and a closing parenthesis."],
            "invariants": ["Every explored prefix has opened at least closed and never exceeds the target length."],
            "edge_cases": ["The smallest valid length n=2.", "A closing parenthesis cannot be the first character.", "Only even target lengths can finish with equal counts."],
            "follow_up": ["Compare this total-length formulation with APIs where n means the number of pairs."],
        },
    },
    {
        "source": "leetcode",
