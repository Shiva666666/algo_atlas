...
from algo_atlas.db import engine, init_db, slugify
...
from algo_atlas.persistence.problems import sync_problem_search
...
LEARNING_PACK: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "failure_reasons": [
            "failure-boundary-or-indexing-error",
            "failure-incorrect-state-transition",
        ],
...
def install_learning_pack(
    target_engine: Engine = engine, source_keys: set[str] | None = None
) -> dict[str, int]:
        ...
        for item in LEARNING_PACK:
            ...
            for taxonomy_slug in item["taxonomy"]:
                ...
                if node.id != primary.id:
                    problem.taxonomy_links.append(
                        ProblemTaxonomy(taxonomy_id=node.id, role=node.kind)
                    )
            for section, bullets in item["notes"].items():
                for position, bullet in enumerate(bullets):
                    problem.note_bullets.append(
                        NoteBullet(section=section, position=position, text=bullet)
                    )
            mistake = MistakeEvent(observation=item["observation"])
            for reason_slug in item["failure_reasons"]:
                mistake.reason_links.append(
                    MistakeEventReason(taxonomy_id=taxonomy[reason_slug].id)
                )
