...
class Problem(Base):
    ...
    primary_subtag_id: Mapped[str] = mapped_column(
    ...
    taxonomy_links: Mapped[list["ProblemTaxonomy"]] = relationship(
        back_populates="problem", cascade="all, delete-orphan"
    )
    note_bullets: Mapped[list["NoteBullet"]] = relationship(
        back_populates="problem", cascade="all, delete-orphan"
    )
    mistake_events: Mapped[list["MistakeEvent"]] = relationship(
        back_populates="problem", cascade="all, delete-orphan"
    )
...
class ProblemTaxonomy(Base):
    ...
    taxonomy_id: Mapped[str] = mapped_column(
        ForeignKey("taxonomy_nodes.id", ondelete="RESTRICT"), primary_key=True
    )
...
class NoteBullet(Base):
    ...
    __table_args__ = (
        Index("idx_note_bullets_problem_section", "problem_id", "section", "position"),
    )
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uid)
    problem_id: Mapped[str] = mapped_column(
...
class MistakeEvent(Base):
    ...
    __table_args__ = (
        Index("idx_mistake_events_problem_occurred", "problem_id", "occurred_at"),
        Index("idx_mistake_events_occurred", "occurred_at"),
    )
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uid)
    problem_id: Mapped[str] = mapped_column(
        ForeignKey("problems.id", ondelete="CASCADE"), nullable=False
    ...
    reason_links: Mapped[list["MistakeEventReason"]] = relationship(
...
class MistakeEventReason(Base):
    ...
    event_id: Mapped[str] = mapped_column(
        ForeignKey("mistake_events.id", ondelete="CASCADE"), primary_key=True
    )
    taxonomy_id: Mapped[str] = mapped_column(
        ForeignKey("taxonomy_nodes.id", ondelete="RESTRICT"), primary_key=True
    )
