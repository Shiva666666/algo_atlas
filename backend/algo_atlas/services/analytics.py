...
from algo_atlas.integrations.git import preview_git
from algo_atlas.models import Problem, TaxonomyNode
from algo_atlas.persistence.problems import load_problem, taxonomy_to_dict
...
def analytics_overview(session: Session) -> dict:
    ...
    problems = [
        problem for problem_id in problem_ids if (problem := load_problem(session, problem_id))
    ]
    taxonomy = session.scalars(
        select(TaxonomyNode).order_by(TaxonomyNode.sort_order, TaxonomyNode.name)
    ).all()
    mains = [node for node in taxonomy if node.kind == "main"]
    failures = [node for node in taxonomy if node.kind == "failure"]
    domains: dict[str, dict] = {
        node.id: {**taxonomy_to_dict(node), "count": 0, "open": 0, "repeat_mistakes": 0}
        for node in mains
    }
    ...
    matrix = [
        {
            ...
            "reason_id": reason.id,
            "reason": reason.name,
            "value": failure_matrix[main.id][reason.id],
        }
        for main in mains
        for reason in failures
        if failure_matrix[main.id][reason.id]
    ]
    ...
    return {
        ...
        "patterns": [
            {"name": name, "count": count} for name, count in pattern_counts.most_common(12)
        ],
        "activity": dates,
...
def atlas_graph(session: Session) -> dict:
    ...
    problem_ids = session.scalars(
        select(Problem.id).order_by(Problem.updated_at.desc()).limit(1500)
    ).all()
    problems = [
        problem for problem_id in problem_ids if (problem := load_problem(session, problem_id))
    ]
        ...
        nodes.append(
            {
                ...
                "name": node.name,
                "kind": node.kind,
                "color": node.color or "#8791a8",
                "value": max(3, counts[node.id] + (18 if node.kind == "main" else 7)),
            }
        )
        if node.parent_id:
        ...
        nodes.append(
            {
                ...
                "name": problem.title,
                "kind": "problem",
                "color": main.color if main else "#8791a8",
                "value": 2 + min(8, len(problem.mistake_events) * 1.5),
                "status": problem.status,
                "difficulty": problem.difficulty,
                "updated_at": problem.updated_at.isoformat() + "Z",
            }
        )
