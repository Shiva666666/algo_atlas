...
from algo_atlas import export_service, sync_service
from algo_atlas.config import settings as default_settings
from algo_atlas.db import build_engine, delete_problem_search, init_db, load_problem, sync_problem_search
from algo_atlas.models import MistakeEvent, MistakeEventReason, NoteBullet, Problem, ProblemTaxonomy, TaxonomyNode
...
def test_incoming_only_changes_apply_and_local_only_changes_are_preserved(two_devices, monkeypatch):
    ...
    with Session(engine_b) as session:
        ...
        problem.note_bullets.append(NoteBullet(section="core_insight", position=0, text="needle-note"))
...
def test_both_changed_requires_current_review_and_keep_local_can_be_published(two_devices, monkeypatch):
    ...
    with Session(engine_a) as session:
        ...
        title_comparison = next(item for item in status["conflicts"][0]["field_comparisons"] if item["field"] == "title")
        ...
        export_before = {
            ...
            for path in shared_exports.rglob("*") if path.is_file()
        }
        ...
        assert "Device A version" in next(shared_exports.rglob("README.md")).read_text(encoding="utf-8") or any(
            "Device A version" in path.read_text(encoding="utf-8") for path in shared_exports.rglob("README.md")
        )
...
def test_invalid_export_and_backup_failure_never_change_local_data(two_devices, monkeypatch):
    ...
    with Session(engine_a) as session:
        ...
        monkeypatch.setattr(sync_service, "create_backup", lambda: (_ for _ in ()).throw(OSError("backup unavailable")))
...
def test_local_deletion_and_duplicate_identity_require_review(two_devices, monkeypatch):
    ...
    with Session(engine_a) as session:
        ...
        replacement = Problem(
            ...
            difficulty="Medium",
            status="Open",
            primary_subtag_id=session.query(TaxonomyNode).filter(TaxonomyNode.kind == "sub").first().id,
        )
...
def test_legacy_conflicts_remain_pending_across_no_op_starts(two_devices, monkeypatch):
    ...
    with Session(engine_a) as session:
        problem_id = first_problem_id(session)
        session.query(sync_service.AppSetting).filter(sync_service.AppSetting.key == sync_service.BASELINE_KEY).delete()
        session.merge(sync_service.AppSetting(key=sync_service.LEGACY_MARKER_KEY, value="{}"))
        delete_problem_search(session, problem_id)
        session.delete(session.get(Problem, problem_id))
        session.commit()
        ...
        assert session.get(sync_service.AppSetting, sync_service.BASELINE_KEY) is None
...
def test_duplicate_incoming_identity_is_rejected_before_writes(two_devices, monkeypatch):
    ...
    readme.write_text(readme.read_text(encoding="utf-8").replace("source_key: other", "source_key: sample"), encoding="utf-8")
...
def test_publish_pull_restart_between_isolated_device_roots(tmp_path, monkeypatch):
    ...
    try:
        ...
        with Session(engine_a) as session:
            ...
            problem.note_bullets.append(NoteBullet(section="core_insight", position=0, text="Published on A"))
            session.add(problem)
            session.flush()
