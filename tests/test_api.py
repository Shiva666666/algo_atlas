...
def test_solved_only_entry_has_no_initial_mistake(client, taxonomy):
    ...
    payload.update(status="Resolved", record_initial_mistake=False, failure_reason_ids=[], observation="")
    ...
    assert client.get("/api/problems", params={"q": "boundary", "status": "Resolved"}).json()["total"] == 1
    overview = client.get("/api/analytics/overview").json()
    assert overview["summary"]["resolved"] == 1
    assert overview["summary"]["repeat_mistakes"] == 0
    repeated = client.post(f"/api/problems/{problem['id']}/mistakes", json={"reason_ids": [taxonomy["failure_reasons"][0]["id"]]})
    assert repeated.status_code == 201
...
def test_sync_status_and_legacy_dry_run_interface(client):
    ...
    assert {"state", "creates", "updates", "conflicts", "review_version", "validation_error"} <= status.json().keys()
