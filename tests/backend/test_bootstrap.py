...
import pytest
from algo_atlas import bootstrap
...
from algo_atlas.services.sync import reconcile_catalog as restore_catalog
...
def test_fresh_database_restores_the_tracked_catalog(bootstrap_environment):
    ...
    expected = json.loads((local_settings.export_dir / "catalog.json").read_text(encoding="utf-8"))[
        "record_count"
    ]
...
def test_real_migrations_and_bootstrap_restore_in_a_temporary_database(
    bootstrap_environment, monkeypatch
):
    from algo_atlas import config

    local_settings, target_engine = bootstrap_environment
    # Migrations use the real source tree, but every writable path remains temporary.
    migration_settings = replace(local_settings, root_dir=default_settings.root_dir)
    monkeypatch.setattr(bootstrap, "settings", migration_settings)
    monkeypatch.setattr(config, "settings", migration_settings)
    result = prepare_local_state(target_engine)
    expected = json.loads((local_settings.export_dir / "catalog.json").read_text(encoding="utf-8"))[
        "record_count"
    ]
    assert result["problem_count"] == expected
...
def test_invalid_exports_do_not_partially_initialize(bootstrap_environment):
    ...
    (local_settings.export_dir / record["path"] / "solution.py").write_text(
        "# corrupted\n", encoding="utf-8"
    )
