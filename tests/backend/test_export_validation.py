...
from algo_atlas.integrations.exports import ExportValidationError, validate_export_catalog
...
def write_catalog(export_dir, catalog):
    (export_dir / "catalog.json").write_text(
...
def test_hash_mismatch_is_rejected(tracked_export):
    catalog = read_catalog(tracked_export.export_dir)
    record = catalog["records"][0]
    (tracked_export.export_dir / record["path"] / "solution.py").write_text(
        "# changed\n", encoding="utf-8"
    )
